    ...
    <tr style="vertical-align:top;">
        ...
        <td style="width:130px; text-align:left;"><% 
            ...
            if res("picture")=true then %>
                <img src="static/images/profiles/<%=res("id")%>.png" style="height:128px;" title="<%=username%>'s avatar" /> <%
            else %>
                <img src="static/images/profiles/<%=gen %>male.png" style="height:128px;" title="<%=username%>'s avatar" /><%
            end if %>
        </td>
    </tr>
    <tr>
        ...
        <div class="listDiv" onclick="location.href='word.asp?id=<%=res("id")%>'">
            <span class="heb" style="float: none; position: relative;">
                ...
                if len(res("imgLink"))>0 then %>
                    <img src="static/images/site/photo.png" alt="לערך זה יש תמונה" title="לערך זה יש תמונה" class="img" style="opacity: 0.6; float: none; position: absolute; top:4px; left: 20px;" /><%
                end if
                if res("wordID") <> "" then %>
                    <img src="static/images/site/audio.png" alt="לערך זה יש סרטון או אודיו" title="לערך זה יש סרטון או אודיו" class="img" style="max-width:16px ; opacity: 0.7; float: none; position: absolute; top:9px; left: 46px;"/><%
                end if
                Select Case res("status")
                Case 1 %>
                    <img src="static/images/site/correct.png" id="ערך זה נבדק ונמצא תקין" alt="ערך זה נבדק ונמצא תקין" title="ערך זה נבדק ונמצא תקין" style="width:15px;opacity:0.4; position: absolute; top:9px; left:5px;" /><%
                Case -1 %>
                    <img src="static/images/site/erroneous.png" id="ערך זה סומן כלא תקין" alt="ערך זה סומן כלא תקין" title="ערך זה סומן כלא תקין" style="width:15px;opacity:0.7; position: absolute; top:9px; left:5px;" /><%
                End Select %>
                </span>
            </span><%
        ...
        Do until res.EOF
            ...
            <div class="listDiv" onclick="location.href='word.asp?id=<%=res("id")%>'">
                ...
                <span class="heb" style="float: none; position: relative;">
                    <%=res("hebrewTranslation")%>
                    <span style="display: inline-block; position: absolute; top: 0px; left:0px;">
                        <img src="static/images/site/hidden.png" alt="מילה זו מוסתרת מעיני הגולשים" title="מילה זו מוסתרת מעיני הגולשים" style="width:15px;opacity:0.7; position: absolute; top:9px; left:5px;" />
                    </span>
                </span><%
                    if len(res("arabic"))>0 then %>
                <span class="arb" style="padding-top: 7px;">
