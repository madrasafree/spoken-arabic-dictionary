...
'response.end
	...
	set f=fs.OpenTextFile(Server.MapPath("2018-GereshFixSearched.csv"),2,true,-1)
...
res.open mySQL, con
	do until res.EOF
		skip = true
		mySQL = "UPDATE wordsSearched SET "
		sep = ""
		dim field
		...
		next
 		...
 		if skip=false then 
 			f.WriteLine(intToStr(res("id"),6) +_
 			",""" +  res("typed") +_
 			""",""" + gereshFix(res("typed")) + vbcrlf) + """"
