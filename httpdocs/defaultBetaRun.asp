    ...
    <div class="search"><%
        if len(strInput)=0 then%>
            <div style="text-align:center; border:0; line-height:1.5; padding:4px;">
                חפשו <span class="heb" style="font-size:larger;">מילה</span>, <span class="arb">כִּלְמֵה</span> <span class="or">או</span> <span class="arb">كلمة</span>
            </div><%
        end if %>
        <form action=defaultBetaRun.asp method="get">
...
if len(strClean)=0 then
    ...
    startTime = timer()
    openDB "arabicWordsBetaRun"
    'OpenDbLogger "arabicWordsBetaRun","O","default","1-FO-RO",strClean
    ...
    closeDB
    'closeDbLogger "arabicWordsBetaRun","C","default","1-FO-RO",durationMs '[db,opType,afPage,opNum,durationMs] %>
    ...
    do until searchNumber=5
        ...
        if skipSndx = false then %>
            ...
            <div class="<%=divClass%>"><%
                startTime = timer()
                openDB "arabicWordsBetaRun"
                'OpenDbLogger "arabicWordsBetaRun","O","default","2",strClean
                mySQL = searchSQL
                res.open mySQL, con
                if not res.EOF then
                    Do until res.EOF
                ...
                closeDB
                'closeDbLogger "arabicWordsBetaRun","C","default","2",durationMs %>
            </div>
...
if len(strDisplay)>0 then
    openDB "arabicSearch"
RESPONSE.WRITE "<div><mark>4o - openDB Search at "& now() &"</mark></div>"
    dim searchID
...
RESPONSE.WRITE "<div><mark>4c - closeDB search at "& now() &"</mark></div>"
