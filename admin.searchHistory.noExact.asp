...
<head>
	<title>50 החיפושים הנפוצים - ללא תוצאה מדויקת</title>
...
startTime = timer()
'openDB "arabicSearch"
openDbLogger "arabicSearch","O","admin.searchHistory.noExact.asp","single",""
...
<div id="container">
	...
	<div id="bread">
		...
		<h1>50 החיפושים הנפוצים - ללא תוצאה מדויקת</h1>
	</div>

	<div><%
		mySQL = "SELECT TOP 50 * FROM wordsSearched WHERE result<>1 AND result<>11 AND searchCount<"&request("downFrom")&" ORDER BY searchCount DESC"
		res.open mySQL, con 
		%>
		<div class="tbl">
			<div>
				...
				<span>מה חיפשו?</span>
				<span>סוג תוצאה</span>
				<span dir="ltr">admin panel</span>
			</div><%
			do until res.EOF %>
				<div>
					<span><%=FormatNumber(res("searchCount"),0)%></span>
					<span><a href="default.asp?searchString=<%=res("typed")%>"><%=res("typed")%></a></span><%
...
'closeDB
closeDbLogger "arabicSearch","C","admin.searchHistory.noExact.asp","single",durationMs,""
