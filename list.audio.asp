...
<table class="tableHeader">
	<tr>
		<td style="width:33%;text-align:right;">
            <%if order="hebrewTranslation" then %> עברית <img src="static/images/site/sort.png" /><%else %> <a href="?order=h">עברית</a><%end if%></td>
		<td style="width:34%;text-align:center;">
            <%if order="arabicWord" then %> ערבית <img src="static/images/site/sort.png" /><%else %> <a href="?order=a">ערבית</a><%end if%></td>
	    <td style="width:33%;" class="tdEng">
            <%if order="pronunciation" then %> הגייה <img src="static/images/site/sort.png" /><%else %> <a href="?order=e">הגייה</a><%end if%></td>
	</tr>
</table><%
	Do until res.EOF %>
        <div class="listDiv" onclick="location.href='word.asp?id=<%=res("id")%>'">
            ...
            if res("mType") then %>
                <img src="static/images/site/audio.png" alt="לערך זה יש סרטון או אודיו" title="לערך זה יש סרטון או אודיו" class="img"  style="max-width:16px ; opacity: 0.8; float: none; position: absolute; top:9px; left: 46px;"/><%
            end if %>
