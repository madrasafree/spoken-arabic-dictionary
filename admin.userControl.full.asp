...
Sub Checkbox(role, mask)
	if (role and mask)<>0 then %>
		<img style="width:16px;height:16px;opacity:.9;" alt="V" src="https://milon.madrasafree.com/static/images/site/correct.png"><%
	else %>
		<img style="width:16px;height:16px;opacity:.3;" alt="X" src="https://milon.madrasafree.com/static/images/site/x.png" /><%
    end if
End Sub
...
startTime = timer()
'openDB "arabicUsers"
openDbLogger "arabicUsers","O","admin.userControl.full.asp","single",""
...
<table class="mainTable">
    <tr style="background:#edc;">
		<td style="max-width:80px;"><a href="admin.userControl.asp?filter=picture">V</a></td>
        <td style="text-align:right;"><a href="admin.userControl.asp">שם אמיתי</a></td>
		<td>שם משתמש</td>
		<td>מייל</td>
		<td><a href="admin.userControl.asp?filter=addWords">add</a></td>
		<td><a href="admin.userControl.asp?order=maxLists desc">lists</a></td>
		<td><a href="admin.userControl.asp?filter=editorWords">עורך</a></td>
		<td><a href="admin.userControl.asp?filter=editorPics">pics</a></td>
		<td><a href="admin.userControl.asp?filter=editorMedia">מדיה</a></td>
		<td><a href="admin.userControl.asp?filter=speaker">דובר</a></td>
		<td><a href="admin.userControl.asp?filter=coder">תכנת</a></td>
		<td>ערבית</td>
		<td>עברית</td>
		<td>להג</td>
		<td>עיר</td>
		<td>קרדיט</td>
		<td>לינק</td>
		<td><a href="admin.userControl.asp?order=joinDateUTC desc">תאריך הצטרפות</a></td>
    ...
    do until res.EOF %>
	    <tr class="userRow"><% 
	        ...
	        if res("picture")=true then %>
	            <td><img src="https://milon.madrasafree.com/static/images/profiles/<%=res("id")%>.png" title="<%=res("userName")%>" style="height:16px;" /></td><%
	        else %>
	            <td><img src="https://milon.madrasafree.com/static/images/profiles/<%=gen %>male.png" title="<%=res("userName")%>" style="height:16px;" /></td><%
	        end if %>
	        ...
	        <td style="text-align:left;"><a href="admin.userEdit.asp?id=<%=res("id")%>"><div style="background:#eeddcc;width:45px;border:solid 1px gray; text-align:center;">עריכה</div></a></td>
	    </tr><%
	    res.movenext
	loop %>
</table><%
...
'closeDB
closeDbLogger "arabicUsers","C","admin.userControl.full.asp","single",durationMs,""
%>
</div>
</body>