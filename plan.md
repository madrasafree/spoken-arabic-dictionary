...
> **SSR-first with React islands** — Django renders pages server-side for SEO, React handles interactive components (search, word editor, lists, games).
...
- [ ] Fix login flow (currently manual/broken for regular users)
- [ ] Fix list creation (currently not functional)
...
- [ ] Export MS Access `.mdb` schema to CSV / SQL
...
- [ ] Inventory existing audio — What's in Clypit? What's embedded from YouTube?
...
1.  **Community lists** — Keep them open to all registered users, or restrict to teachers/team only?
2.  **Hosting** — Where to host the new Django app? (Render, Railway, DigitalOcean, self-hosted VPS?)
3.  **Audio hosting** — Cloudflare R2 (cheap, already using Cloudflare) is a good option — confirm?
4.  **Registration** — When exactly to reopen? After Phase 2 (core pages live) or after Phase 3 (full features)?
5.  **OpenEdX SSO** — Is this a short-term or long-term goal?
